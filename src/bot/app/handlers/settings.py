...
async def change_name_finish(message: Message, state: FSMContext) -> None:
    ...
    async with httpx.AsyncClient() as client:
        user_response = await client.get(f"http://web:80/auth/user/{message.from_user.id}")
        user_data = user_response.json()
        user_data["username"] = new_name

        response = await client.patch(
            f"http://web:80/auth/user/{message.from_user.id}",
            json=user_data
        )

    if response.status_code == status.HTTP_200_OK:
        await message.answer("Имя <b>успешно</b> изменено!", reply_markup=kb.settings_menu)
    else:
